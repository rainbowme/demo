//
//  DrawGraphicController+RegisterDrawMethod.m
//  ApproachIOS
//
...
@implementation DrawGraphicController (RegisterDrawMethod)

- (void)registerDrawMethod
{
    CGSize canvasSize = self.imageView.frame.size;
    canvasSize.width *=2;
    canvasSize.height *=2;
...
self registerId:index name:text forDrawBlock:^(CGContextRef context)
    ...
    [DRAW_GRAPHIC_WITH_BLOCK(kMIDrawLine, @"画一条线") {
        ...
        CGContextSetRGBStrokeColor(context, 0.7f, 0.4f, 0.8f, 0.5f);//线条颜色
        CGContextSetLineWidth(context, 2.0f);
        CGContextMoveToPoint(context, 20, 20);
        CGContextAddLineToPoint(context, canvasSize.width-20, 20);
    ...
    [DRAW_GRAPHIC_WITH_BLOCK(kMIDrawText, @"写文字") {
        ...
        UIFont *font = [UIFont boldSystemFontOfSize:18.0*2];
        NSString *str = @"公司：北京中软科技股份有限公司\n部门：ERP事业部\n姓名：McLiang";
        [str drawInRect:CGRectMake(10, 40, canvasSize.width-20, 300) withFont:font];
        
        UIGraphicsPopContext();
    }];
    // NO.8渐变
    [DRAW_GRAPHIC_WITH_BLOCK(kMIDrawGradientSqure, @"渐变图") {
    ...
    // NO.9四条线画一个正方形
    [DRAW_GRAPHIC_WITH_BLOCK(kMIDrawGroupLineSqure, @"四线矩形") {
        CGContextSetRGBStrokeColor(context, 1.0f, 0.0f, 0.0f, 1.0f);
        UIColor *aColor = [UIColor colorWithRed:0.0f green:1.0f blue:0.0f alpha:0.5f];
    ...
    // NO.12 画一个菱形
    [DRAW_GRAPHIC_WITH_BLOCK(kMIDrawRhomb, @"菱形") {
    ...
    // 画图片
    [DRAW_GRAPHIC_WITH_BLOCK(kMIDrawStar, @"画星星") {
        CGContextSaveGState(context);
        
        
        UIImage *img = [UIImage imageNamed:@"bull_head_green"];
        ...
        CGRect touchRect = CGRectMake(canvasSize.width/2, canvasSize.height/2, img.size.width, img.size.height);
