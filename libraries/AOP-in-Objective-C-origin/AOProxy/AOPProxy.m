...
#pragma clang diagnostic pop
    ...
    NSPredicate *predicate = [NSPredicate predicateWithBlock:^BOOL(AOPInterceptorInfo*info, NSDictionary *x) {
                                  return info.interceptedSelector == aSel;
                              }]
    NSArray *sameSelectors = [methodInterceptors filteredArrayUsingPredicate:predicate]; // Match only items with same selector!
    
    NSPredicate *predicate2 = [NSPredicate predicateWithFormat:@"point == %@", @(InterceptPointStart)];
    invokeSelectors([sameSelectors filteredArrayUsingPredicate:predicate2]);    // Intercept the starting of the method.
    
    [self invokeOriginalMethod:inv];                               // Invoke the original method ...
    
    NSPredicate *predicate3 = [NSPredicate predicateWithFormat:@"point == %@", @(InterceptPointEnd)];
    invokeSelectors([sameSelectors filteredArrayUsingPredicate:predicate3]);    // Intercept the ending of the method.
                    ...
                    interceptorSelector:(SEL)selector
{
    ...
    [methodInterceptors addObject:[AOPInterceptorInfo forSelector:sel
                                                           target:target             // create the interceptorInfo + add to our list
