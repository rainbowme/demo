...
- (void)tapAction:(UITapGestureRecognizer *)ges
{
    ...
    if (CGRectContainsPoint(self.line.frame, location)) {
        ...
        CGFloat HOWMANYDISTANCE = ABS((self.line.selectedLineLength - index * ((self.line.frame.size.width - self.line.ballDiameter) /
                                                                               (self.line.pageCount - 1)))) /
        ((self.line.frame.size.width - self.line.ballDiameter) / (self.line.pageCount - 1));
        
        //背景线条动画
        [self.line animateSelectedLineToNewIndex:index + 1];
        
        // scrollview 滑动
        CGPoint point = CGPointMake(self.bindScrollView.frame.size.width * index, 0);
        [self.bindScrollView setContentOffset:point animated:YES];
